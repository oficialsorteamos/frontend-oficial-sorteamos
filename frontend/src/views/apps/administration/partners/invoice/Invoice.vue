  ...
  <div>
    ...
    >
      ...
      <template #cell(actions)="row">
        ...
        >
          <a :href="row.item.par_url_invoice" target="_blank" rel="noopener noreferrer">
            <feather-icon
              icon="DownloadIcon"
              size="16"
      ...
      <template #cell(partner)="data">
        <div class="text-nowrap">
          <span class="align-text-top">{{ data.item.partner.par_corporate_name }}</span>
        </div>
      </template>

      <template #cell(par_closing)="data">
        <div class="text-nowrap">
          <span class="align-text-top">{{ formatDate(data.item.par_closing) }}</span>
        </div>
      </template>

      <template #cell(par_due)="data">
        <div class="text-nowrap">
          <span class="align-text-top">{{ formatDate(data.item.par_due) }}</span>
        </div>
      </template>
...
export default {
  ...
  created() { 
    ...
    axios
      .get('/api/administration/partner/get-partners')
      .then(response => {
