<template>
  <div>
    ...
    >
      ...
      <b-row
        class="p-1"
        v-if="dialers.length > 0"
      >
        <b-col
          lg="3"
          md="5"
          v-for="dialer in dialers"
          :key="dialer.id"
          ref="refUserListTable"
        >
          <dialer-card-handler 
            :dialer="dialer"
            :fetch-chatbots="fetchDialers"
          />
        </b-col>
      </b-row>
      <b-row
...
import dialerStoreModule from './dialerStoreModule'
import DialerCardHandler from './dialer-card-handler/DialerCardHandler.vue'
...
export default {
  ...
  setup() {
    const DIALER_APP_STORE_MODULE_NAME = 'app-dialer'

    // Register module
    if (!store.hasModule(DIALER_APP_STORE_MODULE_NAME)) store.registerModule(DIALER_APP_STORE_MODULE_NAME, dialerStoreModule)

    // UnRegister on leave
    onUnmounted(() => {
      if (store.hasModule(DIALER_APP_STORE_MODULE_NAME)) store.unregisterModule(DIALER_APP_STORE_MODULE_NAME)
    })
