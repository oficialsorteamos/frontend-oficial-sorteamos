<template>
  ...
  >
    ...
    >
      ...
      <feather-icon 
        :icon="dialer.dia_status == 'I'? 'PlayIcon' : 'PauseIcon'" 
        size="17"
        class="cursor-pointer d-sm-block d-none mr-1"
        v-b-tooltip.hover.v-secondary
        :title="dialer.dia_status == 'A'? $t('voip.voipCardHandler.active') : $t('voip.voipCardHandler.inactive')"
        @click="dialer.dia_status == 'A'? updateStatusVoip('I') : updateStatusVoip('A')"
        v-if="!dialer.pendencies"
        stroke="#28C76F"
      />
      <feather-icon 
        icon="PlayIcon" 
        size="17"
        class="cursor-pointer d-sm-block d-none mr-1"
        :title="dialer.pendencies"
        v-else-if="dialer.pendencies"
        stroke="#FF9F43"
        :id="'pendencies-message'+dialer.id"
      >
      </feather-icon>
      <b-tooltip v-if="dialer.pendencies" :target="'pendencies-message'+dialer.id"><span v-html="dialer.pendencies"> </span> </b-tooltip>
      ...
      <feather-icon icon="SettingsIcon" 
        ...
        @click="openModal('modal-settings-'+dialer.id)"
      />
    </div>
  </div>
  <b-card
  ...
  >
    ...
    <h6 class="text-muted">
      <ul class="list-unstyled">
        <li>
          <small v-if="dialer.dia_description != ''">{{dialer.dia_description}}</small>
          <small v-else>-</small>
        </li>
      </ul>
    </h6>
    ...
    <b-modal
      :id="'modal-settings-'+dialer.id"
      :title="$t('voip.voipCardHandler.addSetting')"
      hide-footer
      size="xl"
    >
      <!-- select 2 demo -->
      <dialer-modal-settings-handler
        :dialer="dialer"
...
import DialerModalSettingsHandler from './dialer-modal-settings-handler/DialerModalSettingsHandler.vue'
