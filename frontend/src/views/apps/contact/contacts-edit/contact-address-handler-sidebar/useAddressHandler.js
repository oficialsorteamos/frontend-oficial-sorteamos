...
export default function useAddressHandler(props, emit) {
  ...
  // taskLocal
  // ------------------------------------------------
  const addressLocal = ref(JSON.parse(JSON.stringify(props.address.value)))
  const resetAddressLocal = () => {
    addressLocal.value = JSON.parse(JSON.stringify(props.address.value))
  }
  watch(props.address, () => {
    resetAddressLocal()
  })
  ...
  const onSubmit = () => {
    const addressData = JSON.parse(JSON.stringify(addressLocal))

    // * If event has id => Edit Event
    // Emit event for add/update event
    if (props.address.value.id) emit('update-address', addressData.value)
    else emit('add-address', addressData.value)
