...
export default function userCalendar() {
  ...
  const grabEventDataFromEventApi = eventApi => {
    const {
      ...
      url,
      // eslint-disable-next-line object-curly-newline
      extendedProps: { calendar, tag, guests, location, description },
      allDay,
    } = eventApi
  ...
  const addEvent = eventData => {
    store.dispatch('calendar/addEvent', { event: eventData }).then(() => {
      ...
      //Se os dados não estão vindo do Modal
      if(eventData.modal == false)
      {
        refetchEvents()
      }
    })
  ...
  watch(selectedCalendars, () => {
    console.log('modificou selectedCalendars')
    refetchEvents()
  })
  ...
  const calendarOptions = ref({
    ...
    */
    ...
    eventClick(clickedEvent) {
      // Caso haja alguma url da reunião, não deixa abrir a url automaticamente no navegador
      clickedEvent.jsEvent.preventDefault(); 
      
      //Caso haja algum link, abre a reunião em uma janela do navegador
      /*if (clickedEvent.event.url) {
        window.open(clickedEvent.event.url);
      }*/

      // * Only grab required field otherwise it goes in infinity loop
      // ! Always grab all fields rendered by form (even if it get `undefined`) otherwise due to Vue3/Composition API you might get: "object is not extensible"
      event.value = grabEventDataFromEventApi(clickedEvent.event)
